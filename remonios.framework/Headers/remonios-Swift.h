...
SWIFT_MODULE_NAMESPACE_PUSH("remonios")
...
- (void)orientationChangedWithNotification:(NSNotification * _Nonnull)notification;
/// 주어진 채널 이름으로 채널에 연결하거나 채널이 없으면 생성한다.
- (void)connectChannelWithChId:(NSString * _Nonnull)chId;
/// 주어진 채널 이름으로 채널에 연결하거나 채널이 없으면 생성한다.
- (void)createBroadcastWithChId:(NSString * _Nullable)chId;
- (void)dummysdp;
- (void)connectChannel;
/// 원격 음성을 안들리게 한다.
- (void)pauseRemoteAudioWithPause:(BOOL)pause;
/// 로컬 영상을 보이거나 안보이게 한다.
- (void)pauseLocalVideoWithPause:(BOOL)pause;
- (void)pauseRemoteVideoWithPause:(BOOL)pause;
/// 토글 방식으로 전면 카메라와 후면카메라를 변경한다.
- (void)switchCamera;
/// 주어진 이름으로 채널 이름을 검새한다. 부분 검색이 가능하다.
- (void)searchWithQuery:(NSString * _Nonnull)query;
